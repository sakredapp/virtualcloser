// POST /api/admin/billing/:repId/cancel-at-week-end
...
export async function POST(_req: Request, ctx: { params: Promise<{ repId: string }> }) {
  ...
  const subIds = await collectSubscriptions(repId)
  if (subIds.length === 0) return NextResponse.json({ ok: false, reason: 'no_subscription' }, { status: 400 })

  const stripe = getStripe()
  const results: { id: string; cancel_at_period_end: boolean }[] = []
  for (const id of subIds) {
    const updated = await stripe.subscriptions.update(id, { cancel_at_period_end: true })
    results.push({ id: updated.id, cancel_at_period_end: updated.cancel_at_period_end })
  }
  await supabase.from('reps').update({ cancel_at_week_end: true }).eq('id', repId)
  await supabase.from('agent_billing').update({ cancel_at_week_end: true }).eq('rep_id', repId)
  await audit({ actorKind: 'admin', action: 'subscription.cancel_at_week_end', repId, notes: `subs: ${subIds.join(', ')}` })

  return NextResponse.json({ ok: true, results })
}

async function collectSubscriptions(repId: string): Promise<string[]> {
