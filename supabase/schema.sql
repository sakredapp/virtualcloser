...
-- Direct phone number for live transfer routing (E.164 or local format).
...
-- ── Multi-role dialer orchestration (concierge / setter / pipeline / live-transfer) ──
-- This layer is rep-scoped so individual plans can use it exactly like
-- enterprise plans, with no team dependency required.
--
-- trigger_kind defines WHAT wakes a workflow:
--   - calendar_*: meeting-confirm/reschedule
--   - crm_stage_changed: e.g. moved to missed_payment / chargeback risk
--   - payment_event: failed payment, chargeback filed
--   - csv_batch: manual lead batch upload
--   - telegram_command: explicit "start pipeline dialer" from Telegram
--
-- dialer_mode defines HOW we talk:
--   - concierge: calendar + schedule management
--   - appointment_setter: outbound booking flow
--   - pipeline: task/workflow-based outbound
--   - live_transfer: transfer-first with fallback
...
-- Enterprise live-transfer availability windows (calendar-style schedule).
-- Reps opt in by publishing windows; dispatch uses these windows to decide
-- whether a member can receive live transfers at call time.
